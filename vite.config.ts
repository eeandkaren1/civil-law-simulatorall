...
function trimLogFile(logPath: string, maxSize: number) {
  try {
    ...
    // Debug collection must never interrupt the client development server.
  }
}
...
/** Development-only browser diagnostics; no runtime dependency is included in production builds. */
function vitePluginDebugCollector(): Plugin {
  return {
    name: "civil-law-debug-collector",
    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use("/__manus__/logs", (req, res, next) => {
        ...
        const handlePayload = (payload: { consoleLogs?: unknown[]; networkRequests?: unknown[]; sessionEvents?: unknown[] }) => {
...
export default defineConfig({
  base: "/civil/",
  plugins: [react(), tailwindcss(), jsxLocPlugin(), vitePluginDebugCollector()],
  resolve: {
    alias: {
  ...
  build: {
    outDir: path.resolve(import.meta.dirname, "dist/public", "civil"),
    emptyOutDir: true,
  },
  server: {
    host: true,
