...
describe("static site identity and metadata", () => {
  it("centralizes the LawVibe brand and contact details", () => {
    ...
    expect(SITE_CONFIG.contactEmail).toBe("guaned0402@gmail.com");
  ...
  it("uses the civil subdirectory and branded metadata", () => {
    ...
    expect(getSeoForPath("/").description.startsWith("LawVibe 法律風｜")).toBe(true);
    expect(getSeoForPath("/scenario/general-001").title).toContain("LawVibe 法律風");
  });
});
